...
// NOTE: no in-memory cache here. On Vercel serverless, each lambda has its own
// memory, so the cache was (a) unreliable across invocations and (b) never
// purged after setSetting() in the admin bot — so the admin's changes didn't
// show for up to 5 minutes. A direct read (single getAllSettings lookup) is
// fast on Accelerate and guarantees fresh data on every call.
